...
public class ServerStateListener implements Listener {
    ...
    public void serverListEvent(ServerListPingEvent e) {
        ...
        if (isMaintenanceMode()) {
            e.setMotd(ChatColor.GOLD + scoreboardName + ChatColor.GRAY + " [1.8.9]" + ChatColor.AQUA + " - v1.5.0-R1" + ChatColor.RED + "\nThe server is currently undergoing " + ChatColor.AQUA + "maintenance." + ChatColor.RED);
            e.setMaxPlayers(24);
            return;
        } else {
            e.setMotd(ChatColor.GOLD + scoreboardName + ChatColor.GRAY + " [1.8.9]" + ChatColor.AQUA + " - v1.5.0-R1" + ChatColor.WHITE + "\nMinecraft " + ChatColor.GOLD + "UHC" + ChatColor.WHITE + " at its finest.");
            e.setMaxPlayers(64);
        }

        if (!(isGameEnabled())) return;
        e.setMotd(ChatColor.GOLD + scoreboardName + ChatColor.GRAY + " [1.8.9]" + ChatColor.AQUA + " - v1.5.0-R1" + ChatColor.GRAY + "\nA game is currently in " + ChatColor.AQUA + "progress!");
    }
}
